    ...
    <div class="p-4">
        <div class="mb-4 flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
                <h1 class="text-xl font-bold text-gray-900 truncate">{{ $location->name }}</h1>
                @if($location->parent)
                    <p class="text-xs text-gray-500 truncate">in {{ $location->parent->fullPath() }}</p>
                @endif
            </div>
        ...
        <div class="flex items-center justify-between mb-2">
            ...
            <a href="{{ route('items.create', ['location_id' => $location->id]) }}"
               class="text-sm text-indigo-600 font-medium">+ Add item</a>
        </div>
